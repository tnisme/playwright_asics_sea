...
import {VariationProduct, VariationProductBuilder} from "../entity/product/VariationProduct";
...
const cardData = fs.readFileSync(path.join(__dirname, '..', 'data', 'card.json'), 'utf8');
...
export class DataTest {
    ...
    static getCustomerInformation(): Customer {
        const customer = customerJSON[process.env.ENVIRONMENT][process.env.LOCATE].customer1;
    ...
    static getVariationProduct1(): VariationProduct {
        ...
        // return VariationProductBuilder.setSize('S').build()
    ...
    static getCard(creditCardType: CreditCardType) {
        const card = cardJSON[process.env.ENVIRONMENT][creditCardType];
