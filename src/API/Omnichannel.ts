...
export class Omnichannel {
  ...
  private static async getToken(request: APIRequestContext): Promise<string> {
    const body = qs.stringify({
      ...
      scope:
        "SALESFORCE_COMMERCE_API:bksc_dev sfcc.inventory.impex-event-log sfcc.inventory.impex-graphs sfcc.inventory.impex-graphs.rw sfcc.inventory.impex-inventory.rw sfcc.inventory.impex-inventory sfcc.inventory.availability sfcc.inventory.availability.rw sfcc.inventory.reservations sfcc.inventory.reservations.rw sfcc.inventory.admin.rw",
    });
    const response: APIResponse = await request.post(
      process.env.BASE_URL_API + process.env.ENDPOINT_GET_TOKEN_API,
      {
        data: body,
        headers: {
  ...
  private static async importImpexApi(
    ...
    const response: APIResponse = await request.post(
      process.env.IMPORT_INVENTORY_API +
        "/inventory/impex/v1/organizations/128d7cd0-1922-45bc-82a1-aec131dff484/availability-records/imports",
      {
        data: body,
        headers: {
  ...
  public static async uploadFile(
    ...
    const response: APIResponse = await request.post(
      process.env.IMPORT_INVENTORY_API + uploadLink,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "*/*",
