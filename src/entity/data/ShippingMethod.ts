...
export class ShippingMethodUtils {
    ...
    private static getFeeForStandardShippingMethod(subTotal: number): number {
        switch (process.env.LOCATE) {
            case 'en_SG': return subTotal < 500 ? 5.9 : 0;
            case 'en_MY': return subTotal < 400.001 ? 20 : 0;
            case 'en_PH': return 150;
            case 'th_TH': return subTotal < 2000 ? 200 : 0;
            case 'vi_VN': return subTotal < 1500000 ? 40000 : 0;
            default: return 0;
        }
    }
    ...
    private static getFeeForNextDayShippingMethod(subTotal: number): number {
        switch (process.env.LOCATE) {
            case 'vi_VN':
            case 'en_SG': return 7.99;
            case 'en_MY': return subTotal < 99.1 ? 7.99 : 0;
            case 'en_PH': return subTotal < 100.01 ? 8 : 0;
            case 'th_TH': return subTotal < 99.01 ? 7.99 : 0;
            default: return 0;
        }
    }
